...
static inline void mark_error(size_t pos) {
  if (err_pos == 0) err_pos = pos;
}
...
static Token get_token(Reader* r) {
  ...
  if (isdigit(c) || c == '.' ||
      ((c == '+' || c == '-') &&
       (isdigit(r->data[r->pos + 1]) || r->data[r->pos + 1] == '.'))) {
    char* end;
...
static void generate_variations(const char* input_file) {
  ...
  char outname[128];
  snprintf(outname, sizeof(outname), "input%d_mehmet_unal_231AMB077.txt",
           file_num);
